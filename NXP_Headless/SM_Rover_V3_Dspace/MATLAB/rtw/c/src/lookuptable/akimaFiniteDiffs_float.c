...
)
{
    ...
    /* For each page: */
    iiFD = 0;
    for (iiF = 0; iiF < numelF; iiF += numelPageF) {

        /* For each (strided) vector on a page: */
        jjFD = iiFD;
        for (jjF = iiF; jjF < iiF + strideF; ++jjF) {
            ...
            for (kk = 1; kk < numelVectorF; ++kk) {
                ...
                if (x != NULL) { /* Used when computing 1-D Akima finite differences FD */
                    ...
                    FD[skkFD] = akimaAbs_float( f2 - f1 ) +
                                akimaAbs_float( f2/2.0f + f1/2.0f );
                }
            }
            ...
            if (x != NULL) { /* Used when computing 1-D Akima finite differences FD */
                 ...
                 */
                FD[jjFD   ] = 2.0f*FD[jjFD   +strideFD] - FD[jjFD   +2*strideFD];
                FD[jjFDend] = 2.0f*FD[jjFDend-strideFD] - FD[jjFDend-2*strideFD];
            }
            else { /* Used when computing the 1-D weights W */
                /* Extrapolate the first weight */
                f2 = F[jjF];
                f1 = 2.0f*f2 - F[jjF+strideF];
