...
)
{
    ...
    memset(s,0,((size_t)nx + 1)*sizeof(float));
    ...
    for (ii = 1; ii < nx+1; ++ii) {
        w[ii] = akimaAbs_float( s[ii] - s[ii-1] ) + 
                akimaAbs_float( s[ii]/2.0f + s[ii-1]/2.0f );
    }
    ...
    w[0] = akimaAbs_float( s2 - s1 ) +
           akimaAbs_float( s2/2.0f + s1/2.0f );
    ...
    for (ii = 0; ii < nx; ++ii) {
         ...
         */
        denW = wL + wU;
        d[ii] = (denW == 0.0f) ? 0.0f : (wU/denW) * sL + (wL/denW) * sU;
    }
}
