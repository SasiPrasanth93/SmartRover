...
void rt_ssSet_slErrMsg(void* S, void* diag) {
    ...
    slsaLogInfo("rt_ssSet_slErrMsg %u", gblDiagnosticLogger);
...
void rt_ssReportDiagnosticAsWarning(void* S, void* diag) {
    slsaLogInfo("rt_ssReportDiagnosticAsWarning %u", gblDiagnosticLogger);
...
void rt_ssReportDiagnosticAsInfo(void* S, void* diag) {
    slsaLogInfo("rt_ssReportDiagnosticAsInfo %u", gblDiagnosticLogger);
    if (gblDiagnosticLogger)
    {
...
{
    slsaLogInfo("rt_RapidInitDiagLoggerDB home: %s, id: %u", dbhome, sid);
    dl_set_home(dbhome);
    gblDiagnosticLogger = dl_init(sid);
    gblBlockPathMemMgrHead = NULL;
    gblBlockPathMemMgrTail = NULL;
    slsaLogInfo("rt_RapidInitDiagLoggerDB done");
}
...
extern void rt_RapidReleaseDiagLoggerDB()
{
    ...
    slsaLogInfo("rt_RapidReleaseDiagLoggerDB Done");
}
...
const char* rt_RAccelReadInportsMatFile(SimStruct* S,
    ...
    {
        double inputTimeTolerance = ssGetExecutionInfo(S)->simulationOptions_.inputTimeTolerance_;
        void *pISigstreamManager = rt_GetISigstreamManager(S);
        rtwISigstreamManagerGetDatasetInputFromMatFile(
	    pISigstreamManager, inportFileName, errmsg, maxErrorBufferSize, errorXMLMode, inputTimeTolerance);
