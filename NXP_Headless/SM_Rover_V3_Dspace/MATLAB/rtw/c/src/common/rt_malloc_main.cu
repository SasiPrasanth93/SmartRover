 ...
 *========================*/
    ...
    #define RTMStopRequested (rtmGetStopRequested(S)) 
#else
    #define RTMStopRequested (S->getStopRequested())
#endif


#if defined(rtmGetErrorStatus)  
    #define RTMGetErrorStatus (rtmGetErrorStatus(S)) 
#else
    #define RTMGetErrorStatus (S->getErrorStatus())
#endif

#if defined(rtmSetErrorStatus)  
    #define RTMSetErrorStatus(status) (rtmSetErrorStatus(S,status)) 
#else
    #define RTMSetErrorStatus(status) (S->setErrorStatus(status)) 
#endif

#if defined(rtmStepTask)  
    #define RTMStepTask(i) (rtmStepTask(S,i)) 
#else
    #define RTMStepTask(i) (S->StepTask(i)) 
#endif


#if defined(rtmTaskCounter)  
    #define RTMTaskCounter(i) (rtmTaskCounter(S,i)) 
#else
    #define RTMTaskCounter(i) (S->TaskCounter(i)) 
#endif

#if defined(rtmCounterLimit)  
    #define RTMCounterLimit(i) (rtmCounterLimit(S,i)) 
#else
    #define RTMCounterLimit(i) (S->CounterLimit(i)) 
#endif


#if defined(rtmGetTFinal)  
    #define RTMGetTFinal (rtmGetTFinal(S)) 
#else
    #define RTMGetTFinal (S->getTFinal()) 
#endif


#if defined(rtmGetRTWExtModeInfo)  
    #define RTMGetRTWExtModeInfo (rtmGetRTWExtModeInfo(S)) 
#else
    #define RTMGetRTWExtModeInfo (S->getRTWExtModeInfo()) 
#endif

#if defined(rtmGetRTWLogInfo)  
    #define RTMGetRTWLogInfo (rtmGetRTWLogInfo(S)) 
#else
    #define RTMGetRTWLogInfo (S->getRTWLogInfo()) 
#endif

#if defined(rtmGetStepSize)  
    #define RTMGetStepSize (rtmGetStepSize(S)) 
#else
    #define RTMGetStepSize (S->getStepSize()) 
#endif
...
# define EXT_MODE 0
    ...
    errmsg = (const char_T *) (RTMGetErrorStatus) ;
    if (errmsg != NULL) {
...
#ifdef UseMMIDataLogging
    const char_T* currentErrorStatus = RTMGetErrorStatus;
    rt_FillStateSigInfoFromMMI(RTMGetRTWLogInfo, &(currentErrorStatus));
    RTMSetErrorStatus(currentErrorStatus);
#endif


    const char_T* currentErrorStatus = RTMGetErrorStatus;
    errmsg = rt_StartDataLogging(RTMGetRTWLogInfo, 
                                 RTMGetTFinal,
                                 RTMGetStepSize,
                                 &(currentErrorStatus));
    RTMSetErrorStatus(currentErrorStatus);
